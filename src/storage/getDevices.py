atmega_io_header = """#if defined (__AVR_AT94K__)
...
#elif defined (__AVR_ATtiny28-asm__)
#  include <avr/iotn28.h>
#elif defined (__AVR_AT90S1200-asm__)
#  include <avr/io1200.h>
#elif defined (__AVR_ATtiny15-asm__)
#  include <avr/iotn15.h>
#elif defined (__AVR_ATtiny12-asm__)
#  include <avr/iotn12.h>
#elif defined (__AVR_ATtiny11-asm__)
#  include <avr/iotn11.h>
#elif defined (__AVR_M3000-asm__)
#  include <avr/iom3000.h"""
# the "-asm" means assembly only. Made it to simplify the classification of devices

splits = atmega_io_header.split(">") # splits at each '>'
# print(len(splits))
# print(splits[-1])

import re
device = re.compile("R_(.+)__") # e.g AVR_(AT94K)__ returns AT94K
file = re.compile("<avr/(.+)") # e.g <avr/(ioat94k.h) returns ioat94k.h

device_and_file:dict = {}
for pair in splits:
    # e.g device_and_file['at94k'] = ioat94k.h
    device_and_file[device.findall(pair)[0].lower()] = file.findall(pair)[0]

from json import dumps
with open('device_and_file.json','w') as f: 
    # small implementation laziness, file gets created in project root
    # so i manually move it to the respective storage folder(s)
    f.write(dumps(device_and_file,indent=2))